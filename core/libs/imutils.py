...
def pil_resize(img, size, order):
    ...
    return np.array(Image.fromarray(img).resize(size[::-1], resample))
...
def crf_inference_label(img, labels, t=10, sxy=50, n_labels=21, gt_prob=0.7):
    ...
    d.addPairwiseBilateral(sxy=sxy, srgb=5, rgbim=np.ascontiguousarray(np.copy(img)), compat=10)
...
def get_strided_size(orig_size, stride):
    ...
    return (torch.div(orig_size[0]-1, stride, rounding_mode='trunc')+1, torch.div(orig_size[1]-1, stride, rounding_mode='trunc')+1)
