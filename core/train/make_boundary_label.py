...
from core.data.data_loader_fetch2 import make_boundary_label_dataset
...
def _work(process_id, infer_dataset, args):
    ...
    with torch.no_grad(), cuda.device(process_id):
        ...
        for iter, (name, img) in enumerate(infer_data_loader):
            ...
            # bg_conf_cam = np.argmin(bg_conf_cam, axis=0)  # 生成背景标签
            ...
            imageio.imwrite(os.path.join(args.cam_vis_dir, img_name + '.png'),
                            conf.astype(np.uint8))  # 边界CAM可视化？

            # 3. convert to boundary label
            window_size = args.window_size
            seg_label = torch.from_numpy(conf).clamp(max=3).cuda()  # 加背景18
            ...
            valid_fg_prop = torch.stack([(seg_label == i).type(torch.float32) for i in range(1, 3)])
            ...
            valid_fg_prop = torch.sort(valid_fg_prop, dim=2, descending=True)[0] # 从大到小排序
...
def run(args):
    n_gpus = torch.cuda.device_count()
    dataset = make_boundary_label_dataset()
