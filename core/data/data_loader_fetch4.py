...
import cv2
from . import transform4cv2_fetch4 as mytf_fetch4
...
def _make_dataset(*dir):
    ...
    if len(dir) == 4:
        ...
        for f in os.listdir(edge_dir):
            if f.endswith(cfg.DATASET.FORMAT_MASK):
                imgs_edge.append(os.path.join(edge_dir, f))
        for f in os.listdir(grayimg_dir):
            if f.endswith(cfg.DATASET.FORMAT_MASK):
                imgs_grayimg.append(os.path.join(grayimg_dir, f))
        return imgs_image, imgs_scr, imgs_edge, imgs_grayimg
...
class MyDataSet(Dataset):
    def __init__(self, dataset_dir, aug_is_need=True, p=None):
        ...
        edge_dir = os.path.join(r'PATH_to_boundary_label\test_results', "boundary_label")
        grayimg_dir = os.path.join(dataset_dir, "gray")
        ...
        self.fetch4_transform = mytf_fetch4.Compose(
            mytf_fetch4.Normalize(mean=mean, std=std),
            mytf_fetch4.Resize(self.train_img_size, self.train_img_size),
            mytf_fetch4.RandomHorizontalFlip(),
            mytf_fetch4.ToTensor()
        )

        self.im_list, self.scr_list, self.edge_list, self.grayimg_list = _make_dataset(im_dir, scr_dir, edge_dir,
                                                                                       grayimg_dir)
    ...
    def __getitem__(self, index):
        ...
        grayimg = cv2.cvtColor(cv2.cvtColor(im, cv2.COLOR_BGR2GRAY), cv2.COLOR_GRAY2BGR).astype(np.float32)[:, :, ::-1]

        if im is None or scr is None:
            print(im_filepath)

        im, scr, edge, grayimg = self.fetch4_transform(im, scr, edge, grayimg)
        ...
        return im, mask, gt, edge, grayimg
